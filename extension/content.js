...
// Generate via the Renderly engine directly (used for cards with refs).
...
function buildDock() {
  ...
  masterRefBtn.title =
    "Reference images used by EVERY card — all cards are then generated by the Renderly engine";
  masterRefBtn.onclick = () => {
    ...
    fileInput.onchange = () => {
      ...
      if (masterRefs.length) {
        setStatus("Global references set — every card will generate via the Renderly engine.");
      }
    };
    fileInput.click();
  };
  ...
  // Generate a card through the Renderly engine: upload global + card refs as
  ...
  const runEngineCard = async (card, index, total, prompt, cardName, cardPrompt) => {
    ...
    setCardStatus(
      card.id,
      versions > 1
        ? `Generating ${versions} versions via Renderly engine…`
        : "Generating via Renderly engine…"
    );
    ...
    for (const gen of gens) {
      if (gen.status !== "done" || !gen.image_url) {
        throw new Error(gen.error || "Generation failed in Renderly");
      }
  ...
  const runCard = async (card, index, total) => {
    ...
    // Flow's UI entirely: refs are uploaded to Renderly and passed to the
